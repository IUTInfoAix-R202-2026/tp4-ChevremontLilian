...
public class FormulaireConnexionViewModel {
  ...
  public void connecterCommand() {
    ...
    statutProperty().setValue("Connexion en cours...");
    if (serviceAuth.connecter(identifiant.getValue(), motDePasse.getValue())) {
      statutProperty().setValue("Bienvenue " + identifiant.getValue() + " !");
    } else {
      statutProperty().setValue("Identifiants incorrects. Vérifiez votre saisie.");
    }
  }
}
